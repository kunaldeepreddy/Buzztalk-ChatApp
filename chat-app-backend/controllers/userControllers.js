...
const getAllUsers = async (req, res) => {
  ...
  if (req.query.search)
    query = { name: { $regex: req.query.search, $options: "i" }, isAdmin: false };
...
const registerUser = async (req, res) => {
  ...
  let user_record = AuthUtil.getObjectReplica(userResponse.toObject(), ["__v", "password"]);
...
const activateOrDeactivateUser = async (req, res) => {
  ...
  let user_record = AuthUtil.getObjectReplica(userRes.toObject(), ["__v", "password", "isAdmin", "isActive"]);
  res.status(200).send({
    status: true,
    message: isActive == true ? "user activated successfully" : "user deactivated successfully",
    data: user_record,
  });
};
...
module.exports = { getAllUsers, searchUsers, registerUser, activateOrDeactivateUser, login, signUp };
